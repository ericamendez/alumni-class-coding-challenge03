...
function balancedNum(number)
{
    ...
    let splitLength = Math.floor(arr.length/2)
    let left = arr.slice(0, splitLength).reduce((acc, num) => acc + Number(num), 0)
    let right = arr.slice(splitLength + 1).reduce((acc, num) => acc + Number(num), 0)

    return right === left ? 'Balanced' : 'Not Balanced'
}

console.log(balancedNum(256523));